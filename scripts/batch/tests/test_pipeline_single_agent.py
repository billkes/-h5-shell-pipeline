...
def test_granular_agent_steps_for_h5_shell() -> None:
    ...
    assert steps.index(SYNC_DISTILLED) < steps.index(AGENT_PLAN_SPEC)
    ...
    assert PLAN_AGENT_STEPS == (AGENT_PLAN_SPEC, AGENT_PLAN_PACK)
    # 1–7 = through lock; 8 = sync.distilled; 9 = first Agent
    assert steps[6] == LOCK_DIMENSIONS
    assert steps[7] == SYNC_DISTILLED
    assert steps[8] == AGENT_PLAN_SPEC
